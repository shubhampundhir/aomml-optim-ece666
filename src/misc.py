...
def load_mnist(num_train=4096, num_val=512):
    """
    Loads MNIST dataset via torchvision with automatic download.
    """
    # Define transform for normalization (grayscale)
    ...
    train_full = torchvision.datasets.MNIST(
        root='./data',       # Data will be stored here
        train=True,          # Training split
        download=True,       # Auto-download if missing
        transform=transform  # Apply transformations
    )

    # Load full test dataset (used as validation set)
    ...
    # Create subsets of desired size
    train_subset, _ = torch.utils.data.random_split(
        train_full,
        [num_train, len(train_full) - num_train]
    )
    val_subset, _ = torch.utils.data.random_split(
        val_full,
        [num_val, len(val_full) - num_val]
    )
...
def plot_losses(losses, val_losses, labels, num_epochs, title, plot_val=False, yscale_log=False, max_epochs=None):
    ...
    for i in range(len(losses)):
        ...
        plt.plot(
            np.linspace(0, num_epochs, len(losses[i].losses)),
            smooth(losses[i].losses, 81),
            label=labels[i],
            alpha=1,
            c=colors[i]
        )
        # Plot training loss (less smoothed, transparent)
        plt.plot(
            np.linspace(0, num_epochs, len(losses[i].losses)),
            smooth(losses[i].losses, 21),
            alpha=0.25,
            c=colors[i]
        )
        ...
        if plot_val:
            plt.plot(
                np.linspace(0, num_epochs, len(val_losses[i].losses)),
                smooth(val_losses[i].losses, 81),
                alpha=1,
                linestyle='--',
                c=colors[i]
            )
    ...
    plt.ylim(0, 3)
    plt.title(f'CNN benchmark on {title.upper()}' if title == 'cifar' else f'MLP benchmark on {title.upper()}')
    
    # Only add legend if valid labels exist
    if labels and any(labels):
        plt.legend(loc='upper right')
    else:
        print("Warning: No valid labels provided for legend. Skipping...")
